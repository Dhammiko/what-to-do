...
module WhatToDo
  ...
  class Event
    ...
    def venue
      @venue ||= Client::EventBriteClient.new.venue_for(venue_id)
    end

    def dsclient
      @dsclient ||= Client::DarkSkyClient.new(latitude: venue.latitude, longitude: venue.longitude, datetime: datetime)
    end

    def forecast
      @forecast ||= WhatToDo::Forecast.new(forecast_json: dsclient.fetch_weather_json, datetime: datetime)
    end
